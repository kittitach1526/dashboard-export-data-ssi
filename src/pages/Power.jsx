...
//   const categories_power = ["Power (5.5)", "Power (6.5)", "Power (7)"];
  ...
  const fetchData_power_Week_5_5 = async () => {
    setLoading(true);
    try {
      const result = await powerService.getPowerWeek_5_5();
  ...
  const fetchData_power_Month_5_5 = async () => {
    setLoading(true);
    try {
      const result = await powerService.getPowerMonth_5_5();
  ...
  //------------------------------------------------------------------------------------------------
    ...
    const fetchData_power_6_5 = async () => {
    setLoading(true);
    try {
      const result = await powerService.getPowerToday_6_5();
  ...
  const fetchData_power_Week_6_5 = async () => {
    setLoading(true);
    try {
      const result = await powerService.getPowerWeek_6_5();
  ...
  const fetchData_power_Month_6_5 = async () => {
    setLoading(true);
    try {
      const result = await powerService.getPowerMonth_6_5();
  ...
  //------------------------------------------------------------------------------------------------
    ...
    const fetchData_power_7 = async () => {
    setLoading(true);
    try {
      const result = await powerService.getPowerToday_7();
  ...
  const fetchData_power_Week_7 = async () => {
    setLoading(true);
    try {
      const result = await powerService.getPowerWeek_7();
  ...
  const fetchData_power_Month_7 = async () => {
    setLoading(true);
    try {
      const result = await powerService.getPowerMonth_7();
  ...
  useEffect(() => {
    if (category === "Power (5.5)" && timePreset === "today") {
      fetchData_power_5_5();
    } else if (category == "Power (5.5)" && timePreset == "week") {
      fetchData_power_Week_5_5();
    }else if (category == "Power (5.5)" && timePreset == "month") {
      fetchData_power_Month_5_5();
    }

    else if (category === "Power (6.5)" && timePreset === "today") {
      fetchData_power_6_5();
    } else if (category == "Power (6.5)" && timePreset == "week") {
      fetchData_power_Week_6_5();
    }else if (category == "Power (6.5)" && timePreset == "month") {
      fetchData_power_Month_6_5();
    }

    else if (category === "Power (7)" && timePreset === "today") {
      fetchData_power_7();
    } else if (category == "Power (7)" && timePreset == "week") {
      fetchData_power_Week_7();
    }else if (category == "Power (7)" && timePreset == "month") {
      fetchData_power_Month_7();
    }
      ...
      <div className="bg-white rounded-3xl shadow-xl p-6 md:p-10 border border-gray-100">
        ...
        <div className="mb-10">
          ...
          {/* <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5">
            {categories_power.map((cat) => (
              <button
                key={cat}
                onClick={() => {
                  setCategory(null); // เคลียร์ฝั่ง Aircom (ถ้าต้องการให้เลือกได้ทีละอย่าง)
                  setCategory_power(cat);
                }}
                className={`py-4 px-6 rounded-2xl font-bold text-lg transition-all ${
                  // ไฮไลท์สีเมื่อ category_power ตรงกับค่านี้
                  category_power === cat
                    ? "bg-orange-500 text-white shadow-lg translate-y-[-2px]" // ใช้สีส้มให้ต่างจาก Aircom
                    : "bg-gray-100 text-gray-400 hover:bg-gray-200"
                }`}
              >
                {cat}
